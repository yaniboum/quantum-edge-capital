...
import random
import requests
import os # <-- NOUVEL OUTIL pour lire les variables secrètes
...
def get_strategy_insight():
    # ... (cette fonction reste simulée et inchangée)
    data = request.get_json()
    risk_level = data.get('risk_level', 'Modéré')
    if risk_level == 'Prudent': insight = "Pour un profil prudent, ma stratégie se concentre sur la préservation du capital..."
    elif risk_level == 'Agressif': insight = "Avec un profil agressif, ma stratégie vise une croissance maximale du capital..."
    else: insight = "Pour un profil modéré, j'adopte une approche équilibrée entre croissance et sécurité..."
    return jsonify({'insight': insight})

@app.route('/api/market-summary')
def get_market_summary():
    """Effectue un appel RÉEL à l'API Gemini en utilisant la clé secrète."""
    
    # LA CORRECTION EST ICI : On lit la clé depuis l'environnement de Render
    api_key = os.getenv('GEMINI_API_KEY')
    
    if not api_key:
        print("ERREUR : La variable d'environnement GEMINI_API_KEY est introuvable.")
        return jsonify({'summary': "Erreur de configuration du serveur : clé API manquante."}), 500

    api_url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-05-20:generateContent?key={api_key}"
    prompt = "Agis comme un analyste financier pour un service de trading algorithmique. Fournis une analyse de marché concise (environ 100-120 mots) pour aujourd'hui. Couvre les mouvements clés sur le Forex (EUR/USD, GBP/JPY) et les matières premières (Or/XAU, Pétrole/WTI). L'analyse doit être factuelle et professionnelle."
    ...
    try:
        ...
        return jsonify({'summary': "L'analyse de marché en temps réel n'est pas disponible (problème de connexion)."}), 500
    except (KeyError, IndexError) as e:
        print(f"Erreur dans le format de la réponse de Gemini : {e}")
        return jsonify({'summary': "L'analyse de marché a retourné une réponse inattendue."}), 500
